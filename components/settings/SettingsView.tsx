...
import { Save, FolderDown, Trash2, Database, Info, HardDrive, FileJson, UploadCloud, HelpCircle, Monitor, ShieldAlert } from 'lucide-react';
    ...
    <div className="p-6 md:p-10 flex flex-col items-center gap-8 overflow-y-auto h-full scrollbar-thin bg-slate-950">
      <div className="max-w-4xl w-full space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="bg-slate-900/40 border-slate-800 p-8 flex flex-col relative overflow-hidden group hover:border-blue-500/30 transition-all duration-500">
            <div className="absolute -right-4 -top-4 w-32 h-32 bg-blue-600/5 rounded-full blur-3xl group-hover:bg-blue-600/10 transition-all"></div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-10 flex-1 italic opacity-80">
                Sử dụng bộ nhớ tạm của trình duyệt. Thích hợp để lưu tiến trình khi chơi liên tục trên cùng một thiết bị.
            </p>
          ...
          {/* Card 2: File Export */}
          <Card className="bg-slate-900/40 border-slate-800 p-8 flex flex-col relative overflow-hidden group hover:border-amber-500/30 transition-all duration-500">
            <div className="absolute -right-4 -top-4 w-32 h-32 bg-amber-600/5 rounded-full blur-3xl group-hover:bg-amber-600/10 transition-all"></div>
            
            <div className="flex items-center gap-5 mb-8">
                <div className="p-4 bg-amber-600/20 rounded-2xl border border-amber-500/30 text-amber-500 shadow-lg shadow-amber-900/20">
                    <FileJson size={32} />
                </div>
                <div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-10 flex-1 italic opacity-80">
                Xuất dữ liệu thành tệp .json để mang sang thiết bị khác hoặc làm bản sao lưu an toàn 100%.
            </p>

            <div className="space-y-3 mt-auto">
                <Button fullWidth onClick={onExportFile} className="gap-2 py-4 bg-blue-600 hover:bg-blue-500 text-xs font-black tracking-widest shadow-xl shadow-blue-900/40 border-none">
        ...
        <div className="pt-8 border-t border-slate-900 text-center space-y-4">
            ...
            <p className="text-[9px] text-slate-700 font-bold uppercase tracking-[0.2em]">Lưu ý: Hành động này sẽ xóa vĩnh viễn LocalStorage và tải lại trang</p>
        </div>
      </div>
    </div>
  );
