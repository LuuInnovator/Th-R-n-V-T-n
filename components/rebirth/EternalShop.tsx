...
import { STAT_UPGRADES, FORGING_UPGRADES, ECONOMY_UPGRADES } from '../../constants/upgrades';
...
export const EternalShop: React.FC<EternalShopProps> = ({ player, onBuyUpgrade }) => {
  const categories = [
    { title: 'Sức Mạnh Bản Thể', icon: Sword, color: 'text-red-400', upgrades: STAT_UPGRADES },
    { title: 'Kỹ Nghệ Đúc Rèn', icon: Hammer, color: 'text-blue-400', upgrades: FORGING_UPGRADES },
    { title: 'Gia Sản & Tốc Độ', icon: Coins, color: 'text-amber-400', upgrades: ECONOMY_UPGRADES },
  ];
    ...
    <div className="h-full flex flex-col bg-slate-950 overflow-hidden animate-fade-in">
      ...
      <div className="bg-slate-900/50 p-6 rounded-3xl border border-slate-800 mb-8 flex flex-col md:flex-row justify-between items-center sticky top-0 z-10 backdrop-blur-xl shrink-0 gap-6">
        <div className="flex items-center gap-5">
            <div className="w-14 h-14 bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl flex items-center justify-center border border-white/10 shadow-lg shadow-purple-900/20">
                <Sparkles size={32} className="text-white" />
            </div>
            <div className="flex flex-col">
                <h3 className="text-2xl font-black text-purple-400 uppercase tracking-tighter italic leading-none">Thiên Phú Vĩnh Cửu</h3>
      ...
      <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin space-y-12 pb-24">
        ...
        <div className="bg-slate-900/20 border border-dashed border-slate-800 p-12 rounded-3xl text-center">
            ...
            <p className="text-xs text-slate-600 font-black uppercase tracking-[0.3em]">Càng tái sinh nhiều lần, giới hạn sức mạnh càng bị phá bỏ</p>
        </div>
      </div>
    </div>
  );
